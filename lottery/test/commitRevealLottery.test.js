...
contract("CommitRevealLottery", (accounts) => {
  ...
  describe("Enter", () => {
    ...
    it("Enter 3 players and check values", async () => {
      ...
      assert.equal(
        ...
        enterAmt,
        "0.01 ETH not sent correctly by account1"
      );
  ...
  describe("Reveal", () => {
    it("Reveal 3 players", async () => {
      ...
      commit = await commitRevealLottery.createCommitment(secret2, {
        from: accounts[2],
      });
      ...
      commit = await commitRevealLottery.createCommitment(secret3, {
        from: accounts[3],
      });
  ...
  describe("withdrawPrice", () => {
    it("withdrawPrice", async () => {
      console.log(">>> before withdrawPrice");
      ...
      await truffleAssert.reverts(
        commitRevealLottery.withdrawPrice({ from: accounts[0] }),
        "You're not the winner"
      );

      let winner = await commitRevealLottery.winner();
      await commitRevealLottery.withdrawPrice({ from: winner });

      console.log(">>> after withdrawPrice");
